...
MASTER-02 quy định dependency giữa các pack, các business domain P0, các quan hệ runtime, rule blocked-if-missing, suppression, fallback và no-hardcode.
...
- ID có bị void, superseded, disputed, failed, blocked hoặc closed hay không.
...
**Source Trace**  
Dữ liệu bắt nguồn từ Source-of-Truth nào.

**Dependency Trace**  
Nghiệp vụ đó phụ thuộc vào domain, pack và Owner Core nào.

**Runtime Trace**  
Runtime Resolver nào đã resolve dữ liệu tại thời điểm thực thi.

**Decision Trace**  
Guard nào đã pass, block, suppress, fallback hoặc handoff.

**Action Trace**  
Action nào được thực hiện: gửi tư vấn, tạo quote, tạo order draft, handoff Messenger, tạo CRM job, tạo payment reference, tạo shipping tracking, tạo complaint case.

**Evidence Trace**  
Bằng chứng nào chứng minh action hoặc dependency đã hoạt động đúng.

**Audit Trace**  
Audit nào ghi nhận trạng thái, người/cơ chế thực hiện, thời điểm, kết quả, lý do, quan hệ trước/sau.

**Release Trace**  
Gate nào được xét, gate nào còn pending, gate nào pass, gate nào block.
...
**Lớp 1 — Business Meaning**  
ID đại diện cho đối tượng, sự kiện, quyết định hoặc bằng chứng nào.

**Lớp 2 — Owner Core**  
Core nào được quyền tạo, quản lý, void, supersede, close hoặc sign-off ID.

**Lớp 3 — Consumer Pack**  
Pack nào được quyền dùng ID để đọc, hiển thị, gửi message, lập quote, tạo order draft, kiểm tra payment, shipping hoặc evidence.

**Lớp 4 — Required Trace Links**  
ID đó bắt buộc phải liên kết với những ID nào để không đứt traceability.

**Lớp 5 — Blocked-if-Missing Rule**  
Nếu thiếu ID hoặc ID không hợp lệ thì Consumer phải dừng, fallback, suppress hoặc handoff như thế nào.
...
- Audit phải ghi nhận mọi xác nhận, từ chối, pending hoặc exception.
...
Nếu thiếu accounting_review_id trong trường hợp cần kế toán xác nhận, trạng thái phải giữ pending review.
...
- Completion report phải trace được trạng thái PASS/PENDING/BLOCKED của từng P0 domain.
...
Nếu P0 gate còn PENDING_EVIDENCE, Completion Report không được chuyển PASS.

Nếu E2E smoke chưa PASS, production ready vẫn là NO.
...
1.  **Correlation Control**  
    Bảo đảm các bước trong cùng một luồng nghiệp vụ được nối với nhau xuyên pack.

2.  **Idempotency Control**  
    Bảo đảm một hành động quan trọng không bị thực hiện lặp ngoài ý muốn.

3.  **Dedup Control**  
    Bảo đảm khách hàng không bị nhận trùng tin, comment không bị trả lời trùng, event không bị xử lý trùng.

4.  **Immutability Control**  
    Bảo đảm ID, snapshot, event, quote, evidence, audit và message lịch sử không bị ghi đè.

5.  **Audit / Evidence Trace Control**  
    Bảo đảm mọi quyết định quan trọng đều có audit, mọi trạng thái PASS đều có evidence, mọi release đều có owner sign-off và completion gate hợp lệ.
...
1.  Public comment → Messenger handoff → AI tư vấn.

2.  Messenger tư vấn → QuoteSnapshot → QuoteCart.

3.  QuoteCart → OrderConfirmationDraft → CustomerConfirmation → OrderCode.

4.  Order → PaymentReference → BankTransferQueue → AccountingReview.

5.  Order → ShippingTrackingEvent → CSKH.

6.  CRM trigger → CRM message job → khách phản hồi → tư vấn tiếp.

7.  Diamond referral link → bind → quote → order → commission.

8.  Member lifecycle → quyền lợi → quote → order.

9.  Health guard / complaint / privacy / counterfeit → suppression → resolution.

10. Evidence → smoke → completion gate → owner sign-off.

11. Live comment → moderation → handoff/private reply/fallback.

12. Product activation → live board → MC AI script → quote/order.
...
1.  Tạo quote_snapshot_id.

2.  Tạo quote_cart_id.

3.  Tạo order_confirmation_draft_id.

4.  Ghi customer_confirmation_id.

5.  Tạo order_code.

6.  Tạo payment_reference.

7.  Đưa giao dịch vào bank_transfer_queue_id.

8.  Ghi accounting_review_id.

9.  Tạo shipping_tracking_event_id.

10. Gửi CRM message.

11. Tạo crm_message_job_id.

12. Thực hiện Messenger handoff.

13. Ghi handoff_delivery_log_id.

14. Ghi live_moderation_event_id.

15. Ghi complaint_case_id.

16. Ghi privacy_case_id.

17. Ghi counterfeit_case_id.

18. Ghi health_guard_case_id.

19. Tạo diamond_bind_id.

20. Tạo commission_event_id.

21. Ghi evidence_id.

22. Ghi owner_signoff_id.

23. Xét completion_gate_id.
...
- Retry của hệ thống không được biến PENDING_EVIDENCE thành PASS.

**38.11. Blocked-if-Missing Idempotency**
...
**Decision Audit**  
Ghi lại guard/resolver đã quyết định pass, block, suppress, fallback hoặc handoff.

**State Audit**  
Ghi lại thay đổi trạng thái của quote, order, payment, shipping, member, commission, complaint, evidence, gate.

**Action Audit**  
Ghi lại action customer-facing như gửi CRM, trả lời Messenger, handoff, tạo order draft, xác nhận đơn.

**Evidence Audit**  
Ghi lại evidence nào được gắn vào smoke/gate, evidence pass/fail/void/superseded.

**Release Audit**  
Ghi lại trạng thái PENDING/PASS/BLOCKED của completion gate, owner sign-off và release control.

**Exception Audit**  
Ghi lại trường hợp thiếu ID, thiếu evidence, thiếu source, thiếu resolver, conflict, retry, dedup, suppression hoặc policy block.

**41.3. Audit bắt buộc cho P0 Domain**

Các domain P0 sau bắt buộc có audit:

1.  Customer Identity / Customer Memory.

2.  CRM 12-Month / Suppression.

3.  Member Lifecycle / Rights / Dispute.

4.  Diamond / Affiliate / Commission.

5.  Product Recommendation.

6.  Pricing / Program / QuoteSnapshot.

7.  Sellable / Product Activation / Golden Hour.

8.  Payment / Bank Transfer / Accounting Review.

9.  Shipping / Tracking / COD / ETA.

10. Order / OrderDraft / CustomerConfirmation / IVR.

11. Gateway / Public Comment / Messenger Handoff / Moderation.

12. Official Contact.

13. Health Boundary / Complaint / Privacy / Counterfeit.

14. Evidence / Completion Report / Gateway Gate / Security.
...
Không được xem Gateway PASS nếu Completion Report còn PENDING_EVIDENCE.
...
Các vi phạm no-hardcode phải được xem là blocker với domain liên quan.

Ví dụ:

- Hardcode tài khoản ngân hàng là blocker Payment Gate.

- Hardcode số điện thoại là blocker Official Contact Gate.

- Hardcode giá là blocker Quote Gate.

- Hardcode hoa hồng là blocker Diamond Gate.

- Hardcode product active là blocker Program/Sellable Gate.

- Hardcode Gateway PASS là blocker Completion Gate.
...
Nếu Payment Core chưa xác nhận, AI không được nói đã thanh toán thành công và phải trace về accounting pending/review.
...
- Gateway không được PASS nếu còn PENDING_EVIDENCE.

- Production Ready vẫn là NO nếu E2E smoke, security evidence, owner sign-off hoặc P0 gate chưa PASS.
...
Một tài liệu governance có thể đạt trạng thái CLEAN FINAL ở tầng tài liệu, nhưng Gateway, Completion Report, Production Ready, Release Approved và Go-live Approved vẫn giữ trạng thái riêng.
...
Nếu một pack cần một ID nhưng không xác định được Owner Core, ID đó phải được đưa về trạng thái BLOCKED tại governance level.
...
- Payment pending nhưng không trace lý do.
...
- Gate không còn PENDING_EVIDENCE.
...
Customer raises issue → Case ID created → Suppression → CRM/Sales blocked → Resolution audit.
...
**59.4. Ý nghĩa trạng thái PENDING_EVIDENCE**

PENDING_EVIDENCE nghĩa là gate chưa đủ bằng chứng accepted.

Một gate ở PENDING_EVIDENCE không được xem là PASS.

Completion Report ở PENDING_EVIDENCE không được xem là Gateway PASS.

Production Ready không được chuyển YES nếu P0 evidence còn pending.

**59.5. Ý nghĩa trạng thái BLOCKED**

BLOCKED nghĩa là có điều kiện P0 chưa đạt hoặc có conflict đang mở.
...
MASTER-03 GOVERNANCE STATUS = PASS  
MASTER-03 CLEAN FINAL = YES  
MASTER-03 TRACEABILITY ID STANDARD = LOCKED  
...
1.  ID Ownership rõ ràng.

2.  ID Generation có điều kiện nghiệp vụ.

3.  ID Immutability bắt buộc.

4.  Snapshot không ghi đè.

5.  Event không sửa lịch sử.

6.  Correlation nối luồng đa pack.

7.  Idempotency chặn xử lý lặp.

8.  Dedup chặn gửi/xử lý trùng.

9.  Audit ghi decision, state, action, fallback, suppression, exception.

10. Evidence có ID, scope, trạng thái, audit và trace.

11. Smoke Mapping bao phủ 16 business domain.

12. Release Control tách governance pass khỏi production ready.

13. Completion Gate không pass khi thiếu evidence.

14. Owner Sign-off bắt buộc cho release.

15. Không hardcode runtime data.

16. Không Consumer nào tự tạo ID làm đứt traceability.

17. Không gọi Gateway PASS chỉ vì tài liệu đã hoàn thành.
...
MASTER-03 kế thừa governance từ MASTER-00, Source-of-Truth từ MASTER-01 và dependency model từ MASTER-02, đồng thời trở thành chuẩn ID bắt buộc cho toàn bộ hệ thống tài liệu kỹ thuật Ginsengfood phía sau.
