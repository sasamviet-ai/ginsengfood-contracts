...
Phụ lục này là lock thực thi ngắn, không override `BẢNG GÔM GIAI ĐOẠN 4.md`. Nếu phụ lục thiếu chi tiết hoặc khác bản gôm, áp dụng bản gôm và xử lý fail-closed.
...
Addendum này là checklist khóa ngắn cho dev/Codex/Copilot trước khi chạy các file 00-08. Bản đầy đủ nằm trong `BẢNG GÔM GIAI ĐOẠN 4.md`; nếu cần chi tiết contract hoặc smoke, đọc Bảng Gôm trước.
...
1. `BẢNG GÔM GIAI ĐOẠN 4.md` đã chứa đầy đủ nguồn và conflict resolution.
2. README Phase 4 trỏ đúng thứ tự đọc.
3. Các contract tối thiểu được liệt kê.
4. P0 smoke/evidence không bị gọi PASS trước khi có artifact.
5. Gateway vẫn `PRODUCTION_BLOCKED`.
