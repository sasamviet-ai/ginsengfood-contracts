# 02-P1 - SẢN PHẨM SKU NGUYÊN LIỆU ĐƠN VỊ TÍNH

Trạng thái: `PRODUCT_SKU_MATERIAL_UOM_CANONICAL`  
Mode: `LIMITED IMPLEMENTATION HANDOFF`  
Mục tiêu: khóa Product/SKU/Material/UOM để công thức và vận hành không suy luận tay.
...
## 2. Material canonical

Material Master phải tách:

1. `material_id`: mã canonical nội bộ.
2. `material_name`: tên chuẩn tiếng Việt.
3. `material_group`: A1/A2/A3/B1/B2/B3.
4. `source_type`: `COMPANY_SOURCE`, `SUPPLIER_SOURCE`, `MIXED_SOURCE`, `OWNER_DEFINED`.
5. `uom_base`: kg/l/ml/unit/pack/box/carton.
6. `qc_required`: true/false.
7. `readiness_required`: true/false.
8. `threshold_policy_code`.
9. `usage_roles`: nhiều role nhưng một material canonical.

## 3. Nhóm nguyên liệu/vật tư chuẩn

| Nhóm | Phạm vi | Policy |
| --- | --- | --- |
| A1 | Quân-Thần-Tá-Sứ, nguyên liệu đặc thù SKU, dược liệu hỗ trợ | +5% MRP, threshold 5 mẻ/SKU hoặc aggregate 20 mẻ |
| A2 | Gạo, rau củ, nước dừa, nước hầm, nền dinh dưỡng | +5% MRP, kiểm dietary claim; không mặc định vegan |
| A3 | Muối, tiêu, tỏi, hành, rễ cần tây, mì chính, hương sâm | +5% MRP, UOM kg/ml |
| B1 | Màng/gói cấp 1 theo SKU | +7%, 15 mẻ/SKU, 115.560 gói quy đổi |
| B2 | Hộp/thùng carton theo SKU | +7%, 28.890 hộp/SKU, 4.815 thùng/SKU |
| B3 | Tem, băng keo, mực in, ribbon, decal, vật tư phụ | 0% mặc định, theo stock policy thực tế |

## 4. UOM policy

| Loại | Đơn vị được dùng | Rule |
| --- | --- | --- |
| Khối lượng | kg, g | Chuẩn hóa về kg cho formula/storage nếu cần |
| Thể tích | lít, ml | Không ép về kg nếu thiếu density policy |
| Bao bì cấp 1 | gói, lọ, hũ, cuộn | Theo packaging profile |
| Bao bì cấp 2 | hộp, thùng | Theo output profile |
| Sản xuất | mẻ, batch, khay | Chỉ dùng trong operational snapshot, không thay UOM material |

## 5. Rule không được sai

1. Không seed SKU nếu thiếu formula_id G1.
2. Không seed material nếu tên chưa canonical hoặc trùng alias.
3. Không scale công thức nếu thiếu UOM conversion.
...
## PHỤ LỤC NỘI DUNG CŨ TRƯỚC REWRITE - 02-P1-2A-PRODUCT-SKU-INGREDIENT-UOM.md
