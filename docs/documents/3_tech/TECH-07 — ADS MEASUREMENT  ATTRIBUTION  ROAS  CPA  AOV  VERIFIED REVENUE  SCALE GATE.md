...
Tự chuyển Payment Pending thành Paid.

Tự chuyển COD Pending thành Revenue.
...
Ads không được tự coi quote, cart, order draft, unpaid order, payment pending, COD pending là revenue.
...
Tự phát Purchase event từ payment pending.
...
Phải đưa vào nhóm Attribution Conflict Pending.
...
Nếu chưa resolve được, đưa vào Conflict Pending.
...
Không được gọi là Completion PASS.
...
Nếu thiếu một trong các điều kiện trên, Scale Gate = BLOCKED hoặc PENDING, không được PASS.
...
Không dùng số liệu pending thay revenue.
...
1.27. P0 Blocker Registry của PHẦN 1/4

Các blocker sau là P0:
...
Ads dùng payment pending làm revenue.

Ads dùng COD pending làm revenue.
...
Nếu xuất hiện bất kỳ P0 blocker nào, TECH-07 status phải là BLOCKED.
...
Evidence xác nhận Diamond/Ads conflict có rule xử lý hoặc bị đưa vào pending.
...
ADS-SMK-003 — Payment pending không tạo revenue
...
ADS-SMK-004 — COD pending không tạo revenue
...
Scale Gate = BLOCKED/PENDING_APPROVAL.

ADS-SMK-009 — Ads + Diamond conflict pending
...
Revenue vào Attribution Conflict Pending.
...
Quote, cart, order draft, payment pending, COD pending, comment, inbox, AI conversation đều có giá trị phân tích, nhưng không phải doanh thu.
...
Module nào nếu fail thì toàn bộ Ads Measurement bị BLOCKED.
...
Tự tạo Purchase event từ payment pending.

Tự tạo Purchase event từ COD pending.
...
Pixel/CAPI tự ghi nhận revenue từ pending order.
...
Campaign có SKU bị Sale Lock thì scale blocked.

Campaign creative chưa approved thì scale blocked.

Campaign có pilot nhưng thiếu evidence thì scale pending.

Campaign không có owner approval thì scale blocked.
...
Claim expired/review pending nhưng vẫn scale.
...
Payment pending bị tính revenue.
...
Payment pending không vào revenue.

COD pending không vào revenue.
...
COD pending được tính là COD success.
...
Payment pending exclusion evidence.

COD pending exclusion evidence.
...
Payment pending không tạo purchase.

COD pending không tạo purchase.
...
Payment Rate tính cả pending payment.
...
Payment pending không làm Payment Success Rate tăng.
...
Pending payment làm revenue.

COD pending làm revenue.
...
Ads + Diamond conflict vào pending nếu chưa có rule.
...
Conflict pending không vào scale revenue.
...
Evidence thiếu → Data Quality Pending.

Data Quality Fail → Scale Gate Blocked.
...
Dashboard hiện Attribution Conflict Pending.
...
Pilot chưa đủ evidence → Pilot Pending.

Pilot data quality fail → Pilot Fail.

Pilot suppression active → Pilot Blocked.
...
Tự scale khi attribution conflict pending.
...
ROAS tốt nhưng data quality fail → Scale Blocked.

ROAS tốt nhưng Sale Lock active → Scale Blocked.

ROAS tốt nhưng creative chưa approved → Scale Blocked.

ROAS tốt nhưng attribution conflict → Scale Pending/Blocked.
...
Không có evidence thì không Completion PASS.
...
Tự gọi pass khi evidence pending.
...
Evidence pending nhưng vẫn pass.
...
Missing evidence → Completion Pending.

Rejected evidence → Completion Fail.

Pending evidence → không Release Ready.
...
2.29. Cross-Module P0 Blocker Registry



2.29.1. Revenue Blocker
...
Dùng payment pending làm revenue.

Dùng COD pending làm revenue.
...
Global Gateway vẫn BLOCKED nhưng bị bỏ qua.
...
Dashboard hiển thị Owner Approval Pending.
...
Có COD pending là tính revenue.
...
DRAFT → REGISTERED → PENDING_APPROVAL → APPROVED_FOR_PILOT → PILOT_RUNNING → PILOT_ACCEPTED → SCALE_REVIEW → SCALE_APPROVED → SCALING_RUNNING
...
### 3.4.5. Campaign P0 Blocker

Campaign bị P0 BLOCKED nếu:
...
Campaign scale khi attribution conflict pending.
...
### 3.5.4. P0 Blocker của ad event

Ad event bị BLOCKED nếu:
...
### 3.6.4. P0 Blocker của Gateway signal

Gateway signal bị BLOCKED nếu:
...
PAYMENT_PENDING_SIGNAL

COD_PENDING_SIGNAL

VERIFIED_REVENUE_PENDING
...
### 3.7.4. P0 Blocker của funnel event

Funnel event bị BLOCKED nếu:
...
ORDER_DRAFT_PENDING_CUSTOMER_CONFIRMATION
...
Quote / Cart / Order Draft lifecycle bị BLOCKED nếu:
...
Không phải payment pending nào cũng là revenue.

Không phải COD pending nào cũng là revenue.
...
PAYMENT_PENDING không phải revenue.
...
COD_PENDING_DELIVERY không phải revenue.
...
Official Order / Payment / COD lifecycle bị BLOCKED nếu:
...
REVENUE_PENDING_COMMERCE

REVENUE_PENDING_PAYMENT_CONFIRMATION

REVENUE_PENDING_COD_RESULT

REVENUE_PENDING_ORDER_VERIFICATION

REVENUE_VERIFIED_BY_COMMERCE

REVENUE_ACCEPTED_FOR_MEASUREMENT

REVENUE_ATTRIBUTION_PENDING

REVENUE_ATTRIBUTED

REVENUE_UNATTRIBUTED

REVENUE_CONFLICT_PENDING
...
Verified Revenue lifecycle bị BLOCKED nếu:
...
Attribution phải chuyển ATTRIBUTION_CONFLICT_PENDING nếu:
...
Conflict pending không được dùng làm scale revenue.



### 3.11.5. P0 Blocker

Attribution bị BLOCKED nếu:
...
Nếu chưa có rule, trạng thái là CONFLICT_PENDING_OWNER_REVIEW.
...
CRM/Ads conflict bị BLOCKED nếu:
...
Mark attribution conflict pending.

Mark revenue unattributed.

Mark campaign scale blocked.
...
Đọc COD pending rồi xem là paid.
...
Dedup lifecycle bị BLOCKED nếu:
...
Không dùng pending data thay verified data.
...
## 3.24. P0 Blocker Registry của PHẦN 3/4



### 3.24.1. Campaign State Blocker
...
### 3.24.4. Attribution State Blocker
...
Conflict pending nhưng vẫn scale.

Attribution bị sửa không evidence.



### 3.24.5. Scale State Blocker
...
### 3.26.5. ADS-LIFE-SMK-005 — Payment pending không phải PAID
...
Cho phép payment pending là PAID.
...
Không có chuyện comment, inbox, AI tư vấn, quote, cart, order draft, payment pending hoặc COD pending được tính ROAS.
...
Khi nào bắt buộc giữ Global Gateway = BLOCKED.
...
Purchase từ order pending được chấp nhận.
...
4.5.4. ADS-FINAL-SMK-015 — COD pending không phải revenue
...
COD pending chỉ là fulfillment/payment pending state.
...
Dashboard không tách COD pending.
...
Commission phát sinh từ conflict pending.

Campaign scale bằng revenue conflict.



4.7.4. ADS-FINAL-SMK-025 — No bind = no Diamond attribution
...
Nếu chưa có rule: conflict pending.
...
Pending/excluded/conflict nằm riêng.
...
Measurement Hold hoặc Scale Blocked.
...
4.9.5. ADS-FINAL-SMK-039 — Evidence chưa accepted thì không Completion PASS
...
Evidence còn pending hoặc missing.

Kỳ vọng

Completion PASS không được cấp.
...
Không có P0 blocker nghiêm trọng.
...
TECH-07 = DOCUMENTATION COMPLETE
...
Conflict pending vẫn dùng scale.
...
TECH-07 = DOCUMENTATION_COMPLETE
...
4.21.1. TECH-08 không được kế thừa sai số liệu
...
4.21.2. TECH-08 được phép kế thừa gì

TECH-08 được phép kế thừa:
...
Ads không được dùng payment pending làm revenue.

Ads không được dùng ảnh chuyển khoản làm PAID.

Ads không được dùng COD pending làm revenue.
...
| --- | --- | --- |
| Principles | Đã khóa Ads Measurement / Verified Revenue / No-Fake-ROAS | DONE |
| Source-of-Truth | Đã khóa Product / Operational / Commerce / AI / Gateway / Ads boundary | DONE |
| Module Contracts | Đã khóa 24 module contract | DONE |
| Lifecycle | Đã khóa Campaign / Event / Revenue / Attribution / Dashboard / Pilot / Scale | DONE |
| Smoke Matrix | Đã khóa 40 smoke final | DONE |
| Evidence Package | Đã khóa evidence groups | DONE |
| Data Quality | Đã khóa pass/hold/fail | DONE |
| Pilot | Đã khóa 7–14 days review | DONE |
| Scale Gate | Đã khóa pass/block rule | DONE |
| Owner Approval | Đã khóa owner review/scope/rejection | DONE |
| Release Handoff | Đã khóa handoff sang TECH-08 và các domain liên quan | DONE |
...
TECH-07 DOCUMENTATION COMPLETE — YES
...
Ads Measurement không được lấy quote, cart, order draft, unpaid order, comment, inbox, AI conversation, payment pending hoặc COD pending làm revenue.
...
TECH-07 — ADS MEASUREMENT / ATTRIBUTION / ROAS / CPA / AOV / VERIFIED REVENUE / SCALE GATE — V1.0 CLEAN FINAL đã hoàn tất về mặt tài liệu.
...
TECH-08 — MC AI LIVE / LIVE SCRIPT RUNTIME / HOSTING INTELLIGENCE / LIVE SALES CONTROL / ADS-SAFE LIVE ORCHESTRATION — V1.0 CLEAN FINAL
