...
IVR CALL GOVERNANCE / ORDER CONFIRMATION STATE MACHINE / NEW CUSTOMER VERIFICATION / INVALID PHONE / NO ANSWER / CUSTOMER CANCEL / TECHNICAL FAILURE / NOTIFICATION HANDOFF / PRIVACY-SAFE CALL RESULT
...
Payment pending chưa có official order.
...
18. P0 BLOCKER REGISTRY — PHẦN 1/4
...
TECH-09 = DOCUMENTATION IN PROGRESS
...
TECH-09 V1.0 khóa baseline MAX_ATTEMPT_PER_ORDER = 2. Nếu sau này đổi sang 3 attempts, phải viết lại bản sạch, không vá lẻ.
...
Eligibility decision: ELIGIBLE / NOT_ELIGIBLE / NEED_HUMAN_REVIEW / BLOCKED.
...
Then result = ELIGIBLE nếu không có blocker.
...
When Phone Validation Resolver chạy
Then module cho phép chuyển sang attempt policy/call attempt nếu không có blocker khác.
...
12. MODULE CONTRACT 10 — NO ANSWER RESOLVER
...
Then module có thể đề xuất IVR verification nếu không có blocker khác.
...
FAIL nếu không có accepted evidence mà Completion PASS.
...
21. P0 BLOCKER REGISTRY — PHẦN 2/4

Các lỗi sau là P0 Blocker ở cấp module:
...
ORDER_CONFIRMATION_NO_ANSWER_PENDING_RETRY
...
ORDER_CONFIRMATION_REQUIRED → ORDER_CONFIRMATION_PENDING.

ORDER_CONFIRMATION_PENDING → ORDER_CONFIRMATION_CALLING.
...
ORDER_CONFIRMATION_CALLING → ORDER_CONFIRMATION_NO_ANSWER_PENDING_RETRY.

ORDER_CONFIRMATION_NO_ANSWER_PENDING_RETRY → ORDER_CONFIRMATION_CALLING nếu còn attempt.

ORDER_CONFIRMATION_NO_ANSWER_PENDING_RETRY → ORDER_CONFIRMATION_NO_ANSWER_MAX_REACHED nếu đủ attempts.

ORDER_CONFIRMATION_PENDING → ORDER_CONFIRMATION_INVALID_PHONE nếu phone invalid.
...
FAIL nếu policy 3 attempts được thêm vá lẻ vào bản 2 attempts.
...
FAIL nếu malicious/purchase-blocked customer vẫn tạo order và gọi IVR như bình thường.
...
No answer max reached nhưng human review đang pending.
...
25. P0 BLOCKER REGISTRY — PHẦN 3/4

Các lỗi sau là P0 Blocker ở cấp lifecycle/state machine:
...
IVR-P3-SMK-002 — Order Draft Blocked
...
Given outcome = NO_ANSWER_MAX_ATTEMPT_REACHED
When IVR bàn giao
Then Core Order State Machine mới quyết định hủy/hold/human review.
...
IVR-FSMK-045 — Không có evidence thì không Completion PASS

Given tài liệu đã viết xong nhưng chưa có accepted evidence
When completion gate chạy
Then Completion PASS vẫn PENDING/BLOCKED.
...
Global Gateway có được mở hay vẫn BLOCKED.
...
Không accepted evidence nhưng Completion PASS.
...
TECH-09 = DOCUMENTATION COMPLETE
...
22.2. Điều kiện nâng từ Documentation Complete lên Completion PASS

Muốn xét Completion PASS cần:
...
Completion PASS = BLOCKED / PENDING_EVIDENCE
...
24. FINAL STATUS REGISTRY

| Hạng mục | Trạng thái sau TECH-09 |
| --- | --- |
| TECH-09 Documentation | DOCUMENTATION COMPLETE |
| Completion PASS | PENDING_EVIDENCE |
| Production Ready | NO |
| IVR Ready | NO |
| Release Ready | NO |
...
Nếu đổi sang 3 attempts phải viết lại bản sạch, không vá lẻ.
