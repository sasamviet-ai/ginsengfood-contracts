...
const operations = [
  ...
  { file: "openapi/ops-core/inventory.v1.yaml", route: "/v1/inventory/allocations", operationId: "listInventoryAllocationsV1", ratePolicy: "ExternalWarehouseRead", permission: "INVENTORY_ALLOCATION_VIEW", schema: "external-inventory-allocation.schema.json", fixture: "inventory-allocations", resource: "inventory.allocations", responseProfile: "inventory", sort: "requestedAt DESC, allocationId DESC", order: [["requested_at", "DESC"], ["allocation_id", "DESC"]], tombstone: "STATE_REVISION_NO_TOMBSTONE", filters: ["sourceObjectId"], fields: ["allocation_id", "allocation_no", "warehouse_id", "warehouse_location_id", "item_type", "item_id", "lot_code", "unit", "source_object_type", "source_object_id", "source_object_no", "allocated_quantity", "status", "requested_at", "confirmed_at", "released_at"] },
...
function assertOperationContract(operation, textOverride) {
  ...
  // Owner 09-09-2026: bon facade kho roi tui doc chung sang ExternalWarehouseRead. Doc tu ban ghi
  // thay vi ghim mot ten: ghim cung se bien moi lan tach tui thanh mot lan sua rai rac.
  const ratePolicy = operation.ratePolicy ?? "ExternalBulkRead";
  assert.match(block, new RegExp("x-rate-limit-policy: " + ratePolicy + "(?![A-Za-z0-9_])"));
...
for (const file of new Set(operations.map(operation => operation.file))) {
  ...
  // Chi file NAO thuc su co operation dung tui rieng moi phai khai khoi do. Khong dieu kien hoa thi
  // sku.v1.yaml - van o tui chung - se bi doi mot khoi no khong dung toi.
  if (operations.some(operation => operation.file === file && operation.ratePolicy === "ExternalWarehouseRead")) {
  assert.match(text, /x-rate-limit-policies:[\s\S]*ExternalWarehouseRead:[\s\S]*sustained_rps_per_principal: 5[\s\S]*burst_per_principal: 10[\s\S]*fleet_rps: 25[\s\S]*queue_limit: 0[\s\S]*required_token_use: service[\s\S]*exhausted_or_unavailable_behavior: FAIL_CLOSED_NO_CACHED_PASS/);
  }
